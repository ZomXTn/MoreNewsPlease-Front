...
const ArticleCard: React.FC<IArticleCardProps> = ({ article }) => {
    ...
    return (
        ...
        >
            <CardHeader avatar={<Avatar sx={{ bgcolor: orange[400] }} src={article?.newspaper?.avatar_url} />} title={article?.title} subheader={new Date(article?.publication_date).toLocaleDateString()} />
            <Divider />
            ...
            <CardActions sx={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                ...
                <Button variant="contained" size="small" onClick={() => navigate(`/articles/${article?.article_id}/read`)}><Typography variant="caption">Lire la suite</Typography></Button>
            </CardActions>
        </Card>
    )
}
