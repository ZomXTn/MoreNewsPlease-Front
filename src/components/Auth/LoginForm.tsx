...
const LoginForm: React.FC = () => {
    ...
    const formik = useFormik({
        ...
        validationSchema: Yup.object().shape({
            email: Yup.string().email("Veuillez saisir une adresse valide").required("Veuillez saisir une adresse valide").min(1, "Veuillez Saisir votre email"),
            password: Yup.string().required("Veuillez saisir un mot de passe valide").min(8, "Veuillez Saisir votre mote de passe")
        })
    })
            ...
            <Box width={{
                ...
                <Paper>
                    <Stack direction={"column"} sx={{
                        ...
                        <TextField id="password" value={formik.values.password} error={formik.errors.password ? true : false} onBlur={formik.handleBlur} onChange={formik.handleChange} label="Mote de Passe" variant="outlined" fullWidth type={showPassword ? "text" : "password"}
                            InputProps={{
                                startAdornment: (
                                    <InputAdornment position="start">
                                        <PasswordIcon />
                                ...
                                endAdornment: (
                                    <InputAdornment position="end">
                                        <Tooltip title={showPassword ? "Masquer Mot de Passe" : "Afficher Mot de Passe"}>
                        ...
                        <Stack direction="row" justifyContent="space-between" alignItems="center" gap={2}>
                            <Link to="/resend-activation" ><Typography variant="caption" textAlign="center"> Nouveau lien d'activation?</Typography></Link>
                            <Link to="/forgot-password"><Typography variant="caption" textAlign="center">Mot de passe oublié?</Typography></Link>
                        </Stack>
