...
const EditArticle: React.FC = () => {
    ...
    const handleSubmit = async (values: IArticle) => {
        try {
            const { newspaper_id } = values.newspaper
            const categories_ids = values.categories.map((cat) => cat.id);
            const data = await updateArticle({ ...values, newspaper_id: newspaper_id || "", categories_ids }).unwrap();
            toast(`L'article a été modifié avec succès`, { type: "success", position: "top-center", autoClose: 3000 })
            setTimeout(() => {
