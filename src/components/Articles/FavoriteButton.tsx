...
const FavoriteButton: React.FC<IFavoriteButton> = ({ articleId, interactions }) => {
    ...
    const handleFavorite = async () => {
        try {
            ...
            toast(`L'article a été ${!favorite ? "ajouté aux favoris" : "retiré des favoris"}`, { type: "success", position: "top-center", autoClose: 2000 })
        } catch (error) {
            toast(`Une erreur s'est produite lors de la ${favorite ? "suppression" : "l'ajout"} de l'article aux favoris`, { type: "error", position: "top-center", autoClose: 2000 })
        }
    }
    return (
