...
const TopbarMenu: React.FC<TopbarMenuProps> = ({ open, anchorEl, handleClose, user }) => {
    ...
    const handleLogout = async () => {
        try {
            await logout()
            toast("Vous êtes déconnecté avec succès", { type: "success", autoClose: 2000, position: "top-center" })
        } catch (e) {
            toast("Erreur lors de la déconnexion", { type: "error", autoClose: 2000 })
        }
    ...
    return (
        <Menu open={open} anchorEl={anchorEl} onClose={handleClose} onClick={(e) => console.log(e)} >
            <Box minWidth={200}>
            ...
            <Divider />
            <IconMenuItem onClick={handleProfile} label="Gestion du Compte" ><EditIcon /></IconMenuItem>
            <IconMenuItem onClick={handleLogout} label="Se Déconnecter" ><LogoutIcon /></IconMenuItem>
        </Menu>
    )
}
