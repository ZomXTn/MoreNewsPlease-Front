...
const articleGridDef = [
    ...
    { key: 'newspaper__name', name: "Journal" },
...
const ArticlesManagement: React.FC = () => {
    ...
    const handleDelete = async (id: string) => {
        try {
            ...
            toast("L'article a été supprimé avec succès", { type: "success", position: "top-center", autoClose: 2000 })
        } catch (error) {
            toast("Une erreur est survenue lors de la suppression de l'article", { type: "error", position: "top-center" })
        }
    }
        ...
        <Container maxWidth="xl">
            <Paper sx={{marginBottom : 2}}>
                <Box padding={3} display="flex" justifyContent="space-between" alignItems="center">
                    <Typography variant="h4" component="span">Gestion des articles</Typography>
                    <Button variant="contained" color="primary" onClick={() => navigate(`/management/articles/create`)}>Créer un article</Button>
                </Box>
            </Paper>
            <Paper>
                            ...
                            {canDisplay &&
                                articles.results.map((article) => (
                                    <TableRow key={article.article_id}>
                                        ...
                                        <TableCell align="center">{article.newspaper?.name}</TableCell>
                                        <TableCell align="center">
